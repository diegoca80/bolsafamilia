...
//var map_color_prop = 'soma_parcelas';
var map_color_prop = 'porcentagem_bf';

//var map_census_prefix = 'R$';
var map_census_prefix = '';

//var map_census_sufix = ',00';
var map_census_sufix = '%';
...
function styleFeature(e) {
  ...
  var map_color_ref = e.getProperty('bfp')[year][month][map_color_prop]
  ...
  return {
    ...
    strokeColor: '#fff',
    zIndex: 1,
    fillColor: getColor(map_color_ref),
    fillOpacity: 0.75,
    visible: true
  };
...
function loadCensus(e) {
  ...
  var map_color_ref = e.feature.getProperty('bfp')[year][month][map_color_prop]
  //console.log('[loadCensus] map_color_ref=' + map_color_ref);

  if (map_color_ref < censusMin) {
    censusMin = map_color_ref;
  }
  if (map_color_ref > censusMax) {
    censusMax = map_color_ref;
  }
...
function mouseInToRegion(e) {
  ...
  function updateCensusBar() {
    ...
    var map_color_ref = e.feature.getProperty('bfp')[year][month][map_color_prop]
    var percent = (map_color_ref - censusMin) / (censusMax - censusMin) * 100;
...
function clickRegion(e) {
  ...
  var geojson_file = 'bfp-' + sigla + '.geojson';
...
function setupCensus() {
  ...
  document.getElementById('census-min').textContent = map_census_prefix + censusMin.toLocaleString() + map_census_sufix;
  document.getElementById('census-max').textContent = map_census_prefix + censusMax.toLocaleString() + map_census_sufix;
